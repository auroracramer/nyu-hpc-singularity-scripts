...
conda update libstdcxx-ng -n soundspaces
conda clean -ya
...
if [[ ! -d "$HABITAT_SIM_DIR" ]]; then
    git clone -b v0.1.7 git@github.com:facebookresearch/habitat-sim.git
fi
...
if [[ ! -d "$SOUNDSPACES_DIR" ]]; then
    ...
    # Checkout latest commit as of Feb 2022 (Sept 27, 2021)
    git checkout f11fef81db0c6b05d42fd062faa4929195de4ddf
    popd
